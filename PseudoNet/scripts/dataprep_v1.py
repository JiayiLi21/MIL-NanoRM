...
from . import helper
from .constants import IVT_KMERS, NUM_NEIGHBORING_FEATURES
...
def partition_into_continuous_positions(arr, window_size=1):
    ...
    #mod_stat_arr = arr["modification_status"]#added

    # map(fun, iter) i.e. (a function to which map passes each element of given iterable, a iterable which is to be mapped)
...
def filter_by_kmer(partition, kmers, window_size):
    ...
    #filtered_mod_stat_arr = mod_stat_arr[mask]#added


    if len(filtered_kmer_arr) == 0:
        return []
    else:
        return filtered_feature_arr, filtered_kmer_arr, filtered_tx_id_arr, filtered_tx_pos_arr #filtered_mod_stat_arr#added
...
def create_features(partition, window_size=1):
    float_arr, kmer_arr, tx_id_arr, tx_pos_arr = partition # modstat change made
    return roll(float_arr, window_size), roll(kmer_arr, window_size), \
        ...
        #mod_stat_arr[window_size: -window_size]#added
...
def filter_events(events, window_size, kmers):
    ...
    #modification_status_arrays = []
    for event_per_read in events:
        ...
        #modification_status_arrays.append(event_per_read[4])
        # print(len(
        # print(len(
        # print(len(transcriptomic_positions_arrays))
        # print(_arrays))
        # addde
    return
...
def parallel_preprocess_tx(eventalign_filepath,out_dir,n_processes,readcount_min,readcount_max, n_neighbors, min_segment_count):
    ...
    #mod_stats = df_eventalign_index['modification_status'].values.tolist()
...
def preprocess_tx(tx_id, data_dict, n_neighbors, min_segment_count, out_paths, locks):  # todo
    ...
    reference_kmer_arrays = []
    transcriptomic_positions_arrays = []
    #modification_status_arrays = [] #added

    for _,events_per_read in data_dict.items(): 
        events_per_read = filter_events(events_per_read, n_neighbors, IVT_KMERS)
        for event_per_read in events_per_read:
            # print(len(event_per_read))
            features_arrays.append(event_per_read[0])
            reference_kmer_arrays.append([combine_sequence(kmer) for kmer in event_per_read[1]])
            transcriptomic_positions_arrays.append(event_per_read[3])
            #modification_status_arrays.append(event_per_read[4]) #added
            # print(len(features_arrays))
            # print(len(reference_kmer_arrays))
            # print(len(transcriptomic_positions_arrays))
            # print(len(modificaiton_status_arrays))
    ...
    if len(features_arrays) == 0:
        ...
        #modification_status_arrays = np.concatenate(modification_status_arrays)
        # print(len(features_arrays))
        # print(len(reference_kmer_arrays))
        # print(len(transcriptomic_positions_arrays))
        # print(len(modificaiton_status_arrays))
    ...
    #modification_status_arrays = np.split(modification_status_arrays[idx_sorted], index[1:])
    ...
    # print(features_arrays.shape)
    # print(positions.shape)
    # print(reference_kmer_arrays.shape)
    for position, features_array, reference_kmer_array  \
            in zip(positions, features_arrays, reference_kmer_arrays):
        # print(position)
