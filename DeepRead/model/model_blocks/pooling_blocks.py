...
### Add a class to return all reads at a given site
class PerReadPooling(InstanceBasedPooling):
    ...
    def __init__(self, input_channel: int, n_reads_per_site: Optional[int] = 20):
        ...
        Initialization function for the class
                ...
                Returns:
                        None
        '''
        super(PerReadPooling, self).__init__(input_channel, n_reads_per_site)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        read_level_prob = self.predict_read_level_prob(x)
        return read_level_prob
